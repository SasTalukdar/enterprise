...
u_ref=np.zeros((29,29))
v_ref=np.zeros((29,29))
mid_i=14
mid_j=14
#r_m=(389.489/111.1)*4
r_m=1
...
for year in range(1982,2020):
    ...
    if len(imd_data)>0:
        ...
        for i in df.index:
            ...
            ga('set lat '+str(df.latitude[i]-3.5)+' '+str(df.latitude[i]+3.5))
            ga('set lon '+str(df.longitude[i]-3.5)+' '+str(df.longitude[i]+3.5))
